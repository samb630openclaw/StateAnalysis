#!/usr/bin/env python3
"""
Nebraska Analysis: Memorial Highways vs Demographics
"""
...
# Paths
demographics_path = "/media/sam/USB DISK/openclaw-capstone-agent/Capstone/Capstone-states/states/nebraska_counties_demographics.csv"
highways_path = "/media/sam/USB DISK/openclaw-capstone-agent/Capstone/Capstone-states/states/nebraska/nebraska-named-highways(1).csv"
output_dir = "/media/sam/USB DISK/openclaw-capstone-agent/results/"

print("=== Nebraska Analysis: Memorial Highways vs Demographics ===")
...
for i, loc in enumerate(highways['Location'].head(10)):
    print(f"{i}: {loc}")
print()
...
# Apply county extraction
highways['extracted_counties'] = highways['Location'].apply(extract_counties)
print("Sample extracted counties:")
for i, counties in enumerate(highways['extracted_counties'].head(10)):
    print(f"{i}: {counties}")
print()

# Count highways per county
all_counties = []
for county_list in highways['extracted_counties']:
...
if county_cols:
    ...
    # Clean county names
    demographics['county_clean'] = demographics[county_col].str.replace(r', Nebraska', '', regex=True)
...
# Save merged data
merged.to_csv(os.path.join(output_dir, 'nebraska_merged_data.csv'), index=False)
print(f"Saved merged data to: {os.path.join(output_dir, 'nebraska_merged_data.csv')}")
print()
...
# Save correlations
corr_df.to_csv(os.path.join(output_dir, 'nebraska_correlation_matrix.csv'), index=False)
print(f"Saved correlations to: {os.path.join(output_dir, 'nebraska_correlation_matrix.csv')}")
print()
...
plt.title('Nebraska: Correlation Matrix - Demographics vs Highway Count', fontsize=16, pad=20)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'nebraska_correlation_heatmap.png'), dpi=300, bbox_inches='tight')
print(f"Saved correlation heatmap to: {os.path.join(output_dir, 'nebraska_correlation_heatmap.png')}")
plt.close()
print()
...
if not ttest_df.empty:
    ...
    # Save t-test results
    ttest_df.to_csv(os.path.join(output_dir, 'nebraska_ttest_results.csv'), index=False)
    print(f"Saved t-test results to: {os.path.join(output_dir, 'nebraska_ttest_results.csv')}")
else:
...
if len(significant_vars) > 0:
    ...
    try:
        ...
        with open(os.path.join(output_dir, 'nebraska_regression_summary.txt'), 'w') as f:
            f.write(str(model.summary()))
        print(f"\nSaved regression summary to: {os.path.join(output_dir, 'nebraska_regression_summary.txt')}")
        ...
        plt.title('Nebraska: Regression Residual Plot')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'nebraska_residual_plot.png'), dpi=300, bbox_inches='tight')
        print(f"Saved residual plot to: {os.path.join(output_dir, 'nebraska_residual_plot.png')}")
        plt.close()
...
for _, row in significant_corrs.head(10).iterrows():
    ...
    plt.title(f'Nebraska: {var} vs Highway Count\nr = {r:.3f}, p = {p:.4f}, n = {len(merged)}')
    plt.tight_layout()
    
    # Create safe filename
    safe_var = var.replace(' ', '_').replace('/', '_')
    filename = f'nebraska_{safe_var}_vs_highways.png'
